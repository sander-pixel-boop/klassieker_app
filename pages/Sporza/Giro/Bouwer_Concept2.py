...
# STEP 2: De Tekentafel
    ...
    if nav_left.button("◀ Vorige Etappe", use_container_width=True, disabled=is_first, help="Je bent al bij de eerste etappe." if is_first else ""):
        st.session_state.aktieve_etappe_idx -= 1
        st.rerun()
    is_last = st.session_state.aktieve_etappe_idx == len(GIRO_ETAPPES) - 1
    if nav_right.button("Volgende Etappe ▶", use_container_width=True, disabled=is_last, help="Je bent al bij de laatste etappe." if is_last else ""):
        st.session_state.aktieve_etappe_idx += 1
        st.rerun()
