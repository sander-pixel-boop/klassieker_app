...
with tab2:
    ...
    if not draft_df.empty:
        ...
        # Bepaal ploeg kolomnaam ('Ploeg' of 'Team' afhankelijk van data)
        ploeg_col = 'Ploeg' if 'Ploeg' in df.columns else 'Team' if 'Team' in df.columns else None

        cols_to_merge = ['Naam', 'Type', 'Prijs', 'EV', 'GC', 'SPR', 'ITT', 'MTN']
        if ploeg_col: cols_to_merge.insert(1, ploeg_col)

        draft_summary = pd.merge(draft_summary, df[[c for c in cols_to_merge if c in df.columns]], on='Naam', how='left')
