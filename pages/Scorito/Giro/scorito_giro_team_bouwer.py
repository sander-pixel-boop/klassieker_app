...
with tab1:
    ...
    with nav_left:
        ...
        if st.button("◀ Vorige", use_container_width=True, disabled=is_first, help="Je bent al bij de eerste etappe." if is_first else ""):
            st.session_state.aktieve_etappe_idx -= 1
            st.rerun()
    with nav_right:
        is_last = st.session_state.aktieve_etappe_idx == len(GIRO_ETAPPES) - 1
        if st.button("Volgende ▶", use_container_width=True, disabled=is_last, help="Je bent al bij de laatste etappe." if is_last else ""):
            st.session_state.aktieve_etappe_idx += 1
            st.rerun()
    with nav_mid:
        etappe_labels = [
...
with tab2:
    ...
    if not huidig_team_namen:
        ...
        if rider_out and rider_in:
            ...
            if st.button("🔄 Bevestig Wissel", disabled=nieuw_budget < 0, type="primary", help="Je hebt onvoldoende budget voor deze wissel." if nieuw_budget < 0 else ""):
