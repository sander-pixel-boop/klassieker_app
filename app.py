...
def load_and_merge_data():
    try:
        ...
        # Onderscheid tussen periode voor en na Roubaix
        ...
        # Bereken EV voor beide delen van het seizoen
...
def solve_knapsack_with_transfers(dataframe, total_budget, min_budget, max_riders, min_per_race, force_list, exclude_list, early_races, late_races, use_transfers):
    ...
    if use_transfers:
        # 3 groepen: Basis (17), Eerste helft (3), Tweede helft (3)
        ...
        # Objective: Maximaliseer EV over het hele seizoen
        prob += pulp.lpSum([
            x[i] * dataframe.loc[i, 'Scorito_EV'] + 
            y[i] * dataframe.loc[i, 'EV_early'] + 
            z[i] * dataframe.loc[i, 'EV_late'] 
            for i in dataframe.index
        ])
        
        # Een renner mag maar 1 rol hebben
        for i in dataframe.index:
            prob += x[i] + y[i] + z[i] <= 1
            if dataframe.loc[i, 'Renner'] in force_list:
                prob += x[i] + y[i] + z[i] == 1 # Moet in de selectie zitten (maakt niet uit waar)
        ...
        # Budgetten (Beginteam <= Budget & Eindteam <= Budget)
        ...
        # Oude, standaard logica (zonder wissels)
...
with col_selection:
    ...
    # Toon Transferplan als dat er is
...
if st.session_state.selected_riders:
    ...
    m1.metric("Budget over (Start)", f"€ {max_bud - current_df['Prijs'].sum():,.0f}")
    m2.metric("Renners", f"{len(current_df)} / {max_ren}")
    
    if st.session_state.transfer_plan:
        ev_start = current_df['EV_early'].sum()
        in_riders = df[df['Renner'].isin(st.session_state.transfer_plan['in'])]['EV_late'].sum()
        base_riders_late = current_df[~current_df['Renner'].isin(st.session_state.transfer_plan['uit'])]['EV_late'].sum()
        m3.metric("Team EV (Incl. wissels)", f"{ev_start + base_riders_late + in_riders:.0f}")
    else:
        m3.metric("Team EV", f"{current_df['Scorito_EV'].sum():.0f}")

    # 3. MATRIX
    st.header("🗓️ 2. Startlijst Matrix (Huidig team)")
    ...
    st.dataframe(pd.concat([totals_row, matrix]), use_container_width=True)

    # 4. KOPMAN
    st.header("🥇 3. Kopman Advies (Huidig team)")
    kop_res = []
    for c in race_cols:
    ...
    st.dataframe(current_df[['Renner', 'COB', 'HLL', 'SPR', 'AVG', 'Total_Races', 'Prijs', 'EV_early', 'EV_late', 'Scorito_EV']].sort_values(by='Scorito_EV', ascending=False), hide_index=True, use_container_width=True)
