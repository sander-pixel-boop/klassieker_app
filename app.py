...
def super_clean(text):
    """Maakt namen extreem simpel voor matching (alleen achternaam)."""
    if pd.isna(text): return ""
    text = str(text).lower()
    text = re.sub(r'^[a-z]\.\s*', '', text) # Initialen
    text = "".join(c for c in unicodedata.normalize('NFD', text) if unicodedata.category(c) != 'Mn') # Accenten
    parts = text.split()
    return parts[-1] if parts else ""

@st.cache_data(ttl=3600)
def scrape_all_pcs():
    """Haalt alle startlijsten op van PCS."""
    results = {}
    headers = {'User-Agent': 'Mozilla/5.0'}
    for abbr, info in RACES_CONFIG.items():
        try:
            resp = requests.get(info["url"], headers=headers, timeout=5)
            soup = BeautifulSoup(resp.content, 'html.parser')
            # Scrape achternamen
            names = [a.text.split()[-1].lower() for a in soup.find_all('a', href=True) if 'rider/' in a['href']]
            results[abbr] = set(names)
        except:
            results[abbr] = set()
    return results
...
def get_status_icon(value):
    """Bepaalt icoon op basis van waarde."""
...
if df_base.empty:
    st.error("Bestanden niet gevonden of namen matchen niet.")
else:
    with st.sidebar:
        st.header("Strategie")
        budget = st.number_input("Budget (Euro)", value=46000000, step=500000)
        source = st.radio("Startlijst Bron:", ["Statisch (CSV)", "Live (PCS)"])
    ...
    if source == "Statisch (CSV)":
        ...
        except: st.warning("startlijsten.csv niet gevonden.")
    else:
        with st.spinner("Startlijsten ophalen..."):
            pcs_lists = scrape_all_pcs()
            for r in races_all:
                df[r] = df['MATCH_KEY'].apply(lambda x: 1 if x in pcs_lists[r] else 0)
    ...
    t1, t2, t3, t4 = st.tabs(["Team Samensteller", "Team Schema", "Programma Overzicht", "Informatie"])
    ...
    with t1:
        ...
        if 'team_idx' in st.session_state:
            ...
            team_disp = team.sort_values('PRIJS_NUM', ascending=False)[['NAAM', 'PRIJS', 'SCORE']]
            st.dataframe(team_disp, hide_index=True)
            st.metric("Budget gebruikt", f"€ {team['PRIJS_NUM'].sum():,.0f}")

    with t2:
        if 'team_idx' in st.session_state:
            team_sel = df.loc[st.session_state['team_idx']].sort_values('PRIJS_NUM', ascending=False).copy()
            disp_matrix = team_sel.copy()
            for r in races_all: 
                disp_matrix[r] = disp_matrix[r].apply(get_status_icon)
            
            st.subheader("Geselecteerd Team Programma")
            st.caption("✅ = Startlijst bevestigd | ? = Onbekend of geen match")
            st.dataframe(disp_matrix[['NAAM'] + races_all], hide_index=True)
            ...
            for r in races_all:
                ...
                if not starters.empty:
                    ...
                    kop_list.append({"Koers": r, "Top 3": " / ".join(top)})
            st.table(pd.DataFrame(kop_list))
        else: st.info("Maak eerst een team.")

    with t3:
        st.subheader("Programma Gehele Markt")
        st.caption("Sorteer op prijs. ✅ = Startlijst bevestigd | ? = Onbekend")
        market_disp = df.sort_values('PRIJS_NUM', ascending=False).copy()
        for r in races_all:
            market_disp[r] = market_disp[r].apply(get_status_icon)
        st.dataframe(market_disp[['NAAM', 'PRIJS'] + races_all], hide_index=True)

    with t4:
        st.write("Data bronnen: WielerOrakel & PCS (Live).")
