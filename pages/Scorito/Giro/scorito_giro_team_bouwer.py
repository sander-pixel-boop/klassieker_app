...
from app_utils.scorito_giro_data import load_giro_data, calculate_giro_ev
...
DB_KOLOM = "scorito_giro_team26_v2"
...
with st.sidebar:
    ...
    st.metric("Budget over", f"€ {50 - totaal_prijs:.2f}M")
    st.metric("Renners",     f"{aantal_renners} / 20")
    if aantal_renners > 20: st.error("🚨 Te veel renners!")
    if totaal_prijs > 50:  st.error("🚨 Budget overschreden!")
...
with tab1:
    ...
    with nav_left:
        ...
        if st.button("◀ Vorige", use_container_width=True, disabled=is_first, help="Je bent al bij de eerste etappe." if is_first else None):
            st.session_state.aktieve_etappe_idx -= 1
            st.rerun()
    with nav_right:
        is_last = st.session_state.aktieve_etappe_idx == len(GIRO_ETAPPES) - 1
        if st.button("Volgende ▶", use_container_width=True, disabled=is_last, help="Je bent al bij de laatste etappe." if is_last else None):
            st.session_state.aktieve_etappe_idx += 1
            st.rerun()
    with nav_mid:
        etappe_labels = [
    ...
    # ── Mini progress bar: all 21 stages as coloured dots (using st.pills) ────────────
    pill_options = []
    for e in GIRO_ETAPPES:
        eid_dot = e["id"]
        n, _ = etappe_samenvatting(str(eid_dot))
        if n > 0:
            pill_options.append(f"🟢 E{eid_dot}")
        else:
            pill_options.append(f"⚪ E{eid_dot}")

    selected_pill_idx = st.session_state.aktieve_etappe_idx
    selected_pill = st.pills("Navigeer snel:", pill_options, default=pill_options[selected_pill_idx], label_visibility="collapsed")

    if selected_pill and selected_pill != pill_options[selected_pill_idx]:
        # User clicked a different pill
        new_idx = pill_options.index(selected_pill)
        st.session_state.aktieve_etappe_idx = new_idx
        st.rerun()
    st.markdown("<small style='color:grey;'>🟢 ingevuld &nbsp; ⚪ leeg</small>", unsafe_allow_html=True)
    ...
    with col_ma1:
        if st.button("🤖 Auto-vul huidige etappe", use_container_width=True, help="Overschrijft de huidige etappe met de Suggesties Top 3"):
            eid_str = str(GIRO_ETAPPES[st.session_state.aktieve_etappe_idx]["id"])
            ...
            st.rerun()
    with col_ma2:
        if st.button("🤖 Auto-vul ALLE 21 etappes", use_container_width=True, help="Overschrijft alle etappes met de Suggesties Top 3 per etappe"):
            for e in GIRO_ETAPPES:
                eid_str = str(e["id"])
                cw = st.session_state.giro_weights_v2[eid_str]
    ...
    with col_info:
        ...
        with st.expander("⚙️ Etappeprofiel weging (pas aan)", expanded=False):
    ...
    if huidig_team_namen:
        ...
        auto_kopman = None
        auto_hint   = "Auto (stel eerst een team in via Tab 2 om Auto te gebruiken)"
    ...
    if gekozen.startswith("🤖"):
        ...
        if huidig_team_namen and gekozen not in huidig_team_namen:
            st.warning(f"⚠️ Let op: **{gekozen}** zit (nog) niet in je definitieve team van 20!")
        else:
...
with tab2:
    ...
    st.divider()
    st.subheader("2. Finaal Team Selecteren (20 Renners)")
    ...
    with c_auto:
        if st.button("🤖 Bereken Optimaal Team", type="primary", use_container_width=True):
            with st.spinner("Optimaal team berekenen... Dit kan even duren."):
                res = solve_giro_team(df, draft_counts=draft_counts, max_bud=50.0, max_ren=20, ev_column="EV")
                if res:
    ...
    st.multiselect(
        "Selecteer handmatig je 20 definitieve renners:",
        ...
        help="Kies maximaal 20 renners voor je selectie."
    )

    c1, c2, c3 = st.columns(3)
    c1.metric("Aantal Renners", f"{aantal_renners} / 20")
    c2.metric("Budget Besteed",  f"€ {totaal_prijs:.2f}M")
    c3.metric("Budget Over",     f"€ {50 - totaal_prijs:.2f}M")
    ...
    if not huidig_team_namen:
        ...
        if rider_out and rider_in:
            ...
            nieuw_budget = round(50 - totaal_prijs + r_out_data['Prijs'] - r_in_data['Prijs'], 2)

            st.markdown(f"**Nieuw budget na wissel:** €{nieuw_budget}M")
            if nieuw_budget < 0:
                st.error("🚨 Deze wissel overschrijdt het budget!")

            if st.button("🔄 Bevestig Wissel", disabled=nieuw_budget < 0, type="primary", help="Je hebt onvoldoende budget voor deze wissel." if nieuw_budget < 0 else None):
...
with tab3:
    if not st.session_state.finaal_team:
        ...
        for etappe in GIRO_ETAPPES:
            ...
            if handmatig_km and handmatig_km not in huidig_team_namen:
                kopman_bron = "🚨 FOUT: Kopman niet in finaal team!"

            with st.expander(
                f"Etappe {etappe['id']}: {etappe['route']} ({etappe['type']}) "
                f"| Kopman: **{effectief_km or '-'}** {kopman_bron}"
            ):
                # Waarschuwing als handmatige kopman buiten de starters of team valt
                if handmatig_km:
                    if handmatig_km not in huidig_team_namen:
                        st.error(f"🚨 **{handmatig_km}** is gekozen als kopman maar zit **niet** in je definitieve team van 20!")
                    elif handmatig_km not in starters_lijst:
                        st.warning(
...
with tab5:
    ...
    **Handmatige kopman (✏️ Handmatig)**
    In Tab 1 kun je per etappe zelf een kopman kiezen uit je 20 renners.
    ...
    2. **Tab 2 – Finaal Team:** Selecteer je definitieve 20 renners of laat de Suggesties ze
